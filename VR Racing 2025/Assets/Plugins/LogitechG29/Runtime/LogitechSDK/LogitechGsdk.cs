...
namespace LogitechG29.Runtime.LogitechSDK
{
    public class LogitechGsdk
    {
        ...
        public static Dijoystate2Engines LogiGetStateCSharp(int index)
        {
            ...
            try
            {
                ret = (Dijoystate2Engines)Marshal.PtrToStructure(LogiGetStateENGINES(index),
                    typeof(Dijoystate2Engines));
            }
            catch (ArgumentException)
            {
                Debug.LogError("Exception catched");
            }
        ...
        [DllImport("LogitechSteeringWheelEnginesWrapper", CharSet = CharSet.Unicode,
            CallingConvention = CallingConvention.Cdecl)]
        public static extern bool LogiGetCurrentControllerProperties(int index,
            ref LogiControllerPropertiesData properties);
        ...
        public struct LogiControllerPropertiesData
        {
            ...
            public override string ToString()
            {
                ...
                builder.AppendLine($"Spring Gain: {springGain}%");
                builder.AppendLine($"Damper Gain: {damperGain}%");
                builder.AppendLine($"Default Spring Enabled: {defaultSpringEnabled}");
                builder.AppendLine($"Default Spring Gain: {defaultSpringGain}%");
                builder.AppendLine($"Combine Pedals: {combinePedals}");
                builder.AppendLine($"Wheel Range: {wheelRange}°");
                builder.AppendLine($"Game Settings Enabled: {gameSettingsEnabled}");
                builder.AppendLine($"Allow Game Settings: {allowGameSettings}");
